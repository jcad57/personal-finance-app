    ...
    <div className="relative grid gap-[20px]">
      <div className="flex items-center justify-between">
        <h2 className="md:text-[length:var(--font-size-xl)] text-[length:var(--font-size-lg)] font-bold text-[var(--grey-900)]">
          {modalProps.modalType === "withdraw" ? `Withdraw from ` : `Add to `}{" "}
          &quot;{modalProps.potName}
          &quot;
        </h2>
        <Image
      ...
      <div className="py-[10px]">
        ...
        <div className="flex justify-between items-center pt-[var(--spacing-sm)]">
          <span
            ...
            style={{
              color:
                modalProps.modalType === "add" ? "var(--green)" : "var(--red)",
            }}
          >
            {((totalSavedAmount / target) * 100).toFixed(2)}%
          </span>
          <span
            className={`text-[var(--grey-500)] text-[length:var(--font-size-xs)]`}
          >
...
const ProgressBar = ({
  ...
  const colorBarWidth =
    modalType === "add"
      ...
      : ((currentSaved - amountToWithdraw) / target) * 100;
